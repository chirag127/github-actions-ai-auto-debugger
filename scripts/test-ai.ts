...
`;
  ...
  try {
    ...
    if (fixedContent.includes("```")) {
      console.warn(
        "⚠️ Warning: The AI included markdown fences despite the prompt explicitly forbidding it.",
      );
    } else if (fixedContent.includes("is the result")) {
      console.warn("⚠️ Warning: The AI didn't actually fix the bug.");
    } else {
      console.log(
        "🎉 SUCCESS! The AI correctly fixed the code without any markdown or extra text.",
      );
    }
  } catch (error) {
